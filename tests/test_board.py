...
def test_create_assigns_ticket_history_and_owner(board: status.Board) -> None:
...
def test_find_accepts_slug_and_ticket_spellings(board: status.Board, reference: str) -> None:
    board.create("alpha", "Alpha", "backlog", "claude")
    assert board.find(reference).id == "alpha"


def test_create_refuses_duplicate_id(board: status.Board) -> None:
    board.create("alpha", "Alpha", "backlog", "claude")
    with pytest.raises(status.BoardError, match="duplicate id"):
        board.create("alpha", "Another", "backlog", "claude")


def test_create_refuses_forbidden_lane(board: status.Board) -> None:
    with pytest.raises(status.BoardError, match="may not create"):
        board.create("alpha", "Alpha", "completed", "claude")


def test_move_records_legal_transition(board: status.Board) -> None:
...
def test_representative_allowed_edges(
        board: status.Board, actor: str, source: str, destination: str) -> None:
    item = status.Item("alpha", "Alpha", source, ticket=1, owner="claude")
...
def test_representative_forbidden_edges(
    ...
    item = status.Item("alpha", "Alpha", source, ticket=1, owner="claude")
    board.items.append(item)
    with pytest.raises(status.BoardError):
        board.move("alpha", destination, actor)
    assert item.state == source and item.history == []


def test_move_refuses_claude_signoff(board: status.Board) -> None:
    item = status.Item("alpha", "Alpha", "ready_for_review", ticket=1, owner="claude")
    board.items.append(item)
    with pytest.raises(status.BoardError, match="not to completed"):
        board.move("alpha", "completed", "claude")
    assert item.state == "ready_for_review"
    assert item.history == []


def test_assignment_and_priority_are_non_lane_history(board: status.Board) -> None:
    board.create("alpha", "Alpha", "backlog", "claude")
...
def test_comment_refuses_blank_text(board: status.Board) -> None:
    board.create("alpha", "Alpha", "backlog", "claude")
    with pytest.raises(status.BoardError, match="needs text"):
        board.comment("alpha", "  ", "terry")


def test_human_comment_creates_reference_but_bot_comment_does_not(
        board: status.Board) -> None:
    board.create("alpha", "Alpha", "backlog", "claude")
...
def test_link_is_stored_once_and_inverse_is_derived(board: status.Board) -> None:
    board.create("alpha", "Alpha", "backlog", "claude")
    board.create("beta", "Beta", "backlog", "claude")
...
def test_parent_cycle_is_refused_and_rolled_back(board: status.Board) -> None:
    board.create("alpha", "Alpha", "backlog", "claude")
    board.create("beta", "Beta", "backlog", "claude")
    board.set_parent("beta", "alpha", "claude")

    with pytest.raises(status.BoardError, match="parent cycle"):
        board.set_parent("alpha", "beta", "claude")
    assert board.find("alpha").parent is None
    assert board.find("beta").parent == "alpha"


def test_lanes_sort_by_priority_then_creation_then_ticket(board: status.Board) -> None:
    newer = status.Item(
        "newer", "Newer", "backlog", ticket=2, priority="P2", owner="claude",
        history=[status.Change("2026-01-02T00:00:00+00:00", "backlog", "claude")])
    older = status.Item(
        "older", "Older", "backlog", ticket=1, priority="P2", owner="claude",
        history=[status.Change("2026-01-01T00:00:00+00:00", "backlog", "claude")])
    urgent = status.Item(
        "urgent", "Urgent", "backlog", ticket=3, priority="P1", owner="claude",
        history=[status.Change("2026-01-03T00:00:00+00:00", "backlog", "claude")])
...
def test_verify_detects_direct_state_write(board: status.Board) -> None:
    board.create("alpha", "Alpha", "backlog", "claude")
    board.find("alpha").state = "completed"
    assert any("history ends" in problem for problem in board.verify())


def test_verify_detects_broken_history(board: status.Board) -> None:
    item = status.Item(
        "alpha", "Alpha", "completed", ticket=1, owner="claude",
        history=[
            status.Change("2026-01-01T00:00:00+00:00", "backlog", "claude"),
            status.Change("2026-01-02T00:00:00+00:00", "completed", "claude",
...
def test_verify_detects_illegal_recorded_transition(board: status.Board) -> None:
    item = status.Item(
        "alpha", "Alpha", "completed", ticket=1, owner="claude",
        history=[
            status.Change("2026-01-01T00:00:00+00:00", "backlog", "claude"),
